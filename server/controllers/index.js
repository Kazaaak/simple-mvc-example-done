...
// A function for updating the last dog added to the database.
...
const searchDogName = async (req, res) => {
  ...
  let doc;
  try {
    // I borrowed formatting for incrementing from this site, though I didn't get it to work
    // https://stackoverflow.com/questions/41444213/how-do-i-increment-a-number-value-in-mongoose
    doc = await Dog.findOneAndUpdate({ name: req.query.name }, { $inc: { 'req.body.age': 1 } }).exec();
  } catch (err) {
    // If there is an error, log it and send the user an error message.
    console.log(err);
    return res.status(500).json({ error: 'Something went wrong' });
